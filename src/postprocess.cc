...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "./model/classes_9_labels_list.txt"
#define LABEL_NALE_TXT_PATH_ALT1 "../rk3588_linux/model/classes_9_labels_list.txt"
#define LABEL_NALE_TXT_PATH_ALT2 "rk3588_linux/model/classes_9_labels_list.txt"
        ...
        for (int w = 0; w < grid_w; w++) {
            for (int a = 0; a < anchor_per_branch; a++) {
                ...
                if (box_confidence >= thres_i8) {
                    ...
                    float limit_score = box_conf_f32;
...
int init_post_process()
{
    const char* label_paths[] = {
        LABEL_NALE_TXT_PATH,
        LABEL_NALE_TXT_PATH_ALT1,
        LABEL_NALE_TXT_PATH_ALT2,
    };

    for (const char* path : label_paths)
    {
        if (loadLabelName(path, labels) == 0)
        {
            return 0;
        }
    }

    printf("Load label file failed on all fallback paths.\n");
    return -1;
}
