...
static thread_local yolov5_perf_t g_last_perf = {0.0f, 0.0f, 0.0f};

yolov5_perf_t get_last_yolov5_perf()
{
    return g_last_perf;
}
...
#if defined(RKNN_NPU_CORE_0_1_2)
    ret = rknn_set_core_mask(ctx, RKNN_NPU_CORE_0_1_2);
    if (ret != RKNN_SUCC)
    {
        printf("rknn_set_core_mask(all cores) fail! ret=%d\n", ret);
    }
#elif defined(RKNN_NPU_CORE_AUTO)
    ret = rknn_set_core_mask(ctx, RKNN_NPU_CORE_AUTO);
    if (ret != RKNN_SUCC)
    {
        printf("rknn_set_core_mask(auto) fail! ret=%d\n", ret);
    }
#endif
...
{
    std::chrono::steady_clock::time_point preprocess_start;
    int ret;
    ...
    g_last_perf.preprocess_ms = std::chrono::duration<float, std::milli>(preprocess_end - preprocess_start).count();
    g_last_perf.run_ms = std::chrono::duration<float, std::milli>(run_end - run_start).count();
    g_last_perf.postprocess_ms = std::chrono::duration<float, std::milli>(post_end - post_start).count();
...
#else
    static thread_local std::vector<unsigned char> tl_preprocess_buf;
    if (tl_preprocess_buf.size() < static_cast<size_t>(dst_img.size)) {
        tl_preprocess_buf.resize(dst_img.size);
    }
    dst_img.virt_addr = tl_preprocess_buf.data();
#endif
